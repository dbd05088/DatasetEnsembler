...
class EarlyTrain(CoresetMethod):
    ...
    def run(self):
        ...
        # self.model = self.model.vision_model.to(self.args.device)
        
        if self.args.model == "CLIP":
            # CLIP
            config_vision = CLIPVisionConfig()
            self.model = nets.clip.CLIPViT(config_vision)
            pretrained_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
            self.model.load_state_dict(pretrained_model.vision_model.state_dict(), strict=False)
            self.model.visual_projection.load_state_dict(pretrained_model.visual_projection.state_dict())
            self.model.head = nn.Linear(512, self.num_classes)
            self.model = self.model.to(self.args.device)
            
        elif self.args.model == "DINO_base":
            # DINO
            self.model = nets.dino.vit_base(patch_size = 16, num_classes = self.num_classes).to(self.args.device)
            self.model.load_state_dict(torch.hub.load('facebookresearch/dino:main', 'dino_vitb16').state_dict(), strict=False)
            # self.model = torch.hub.load('facebookresearch/dino:main', 'dino_vitb16')
            # self.model.head = nn.Linear(768, self.num_classes)
            # self.model = self.model.to(self.args.device)

        elif self.args.model == "DINO_small":
            # DINO
            self.model = nets.dino.vit_small(patch_size = 8, num_classes = self.num_classes).to(self.args.device)
            self.model.load_state_dict(torch.hub.load('facebookresearch/dino:main', 'dino_vits8').state_dict(), strict=False)
            # self.model = torch.hub.load('facebookresearch/dino:main', 'dino_vitb16')
            # self.model.head = nn.Linear(768, self.num_classes)
            # self.model = self.model.to(self.args.device)
        ...
        print(f'./pretrained/{self.args.dataset}_{self.args.model}_lr_{self.args.selection_lr}_epoch_{self.args.selection_epochs}.pt')
        if os.path.exists(f'./pretrained/{self.args.dataset}_{self.args.model}_lr_{self.args.selection_lr}_epoch_{self.args.selection_epochs}.pt'):
            print('\nLoading the pre-trained model of {model_name}'.format(model_name = self.model_name))
            self.model.load_state_dict(torch.load(f'./pretrained/{self.args.dataset}_{self.args.model}_lr_{self.args.selection_lr}_epoch_{self.args.selection_epochs}.pt'))
            ...
            for epoch in range(self.epochs):
                ...
                if self.dst_test is not None and self.args.selection_test_interval > 0 and (epoch + 1) % self.args.selection_test_interval == 0:
                    self.test(epoch)
                self.after_epoch()
            torch.save(self.model.state_dict(), f'./pretrained/{self.args.dataset}_{self.args.model}_lr_{self.args.selection_lr}_epoch_{self.args.selection_epochs}.pt')
